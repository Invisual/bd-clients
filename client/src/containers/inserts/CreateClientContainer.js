...
class CreateClientContainer extends Component{
    ...
    getClientData = () => {
        ...
        axios.get(`/api/clients/basic/${id}`, { headers: { Authorization: AuthStr } })
        .then(res => {
    ...
    insertClient = (e) => {
        ...
        var token = JSON.parse(localStorage.getItem('token'));
        var AuthStr = 'Bearer ' + token;
        axios.post('/api/clients/', data, { headers: { Authorization: AuthStr } })
        .then(res => {
    ...
    editClient = (e) => {
        ...
        var token = JSON.parse(localStorage.getItem('token'));
        var AuthStr = 'Bearer ' + token;
        axios.put('/api/clients/', data, { headers: { Authorization: AuthStr } })
        .then(res => {
            if(res.data.affectedRows){
                Swal.fire({
                    type: 'success',
    ...
    componentDidMount(){
        if(this.props.type === 'edit'){ this.getClientData();}
    }
