    ...
    return (
        <div className="mytasks-container widget cards-container">
          ...
          {props.tasks.map(task => {
              return <SingleTask key={task.id_task} id={task.id_task} title={task.title_task} state={task.state} hourState={1} projectState={task.ref_id_project} stateVal={task.ref_id_user_task_status} stateTitle={task.name_user_task_status}/>
          })}
