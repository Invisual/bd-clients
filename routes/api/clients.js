          ...
          connection.query(
            ...
            function (err, results, fields) {
              if (err) {
                ...
                taskHours =
                  results[0][0].total_hours !== null
                    ? moment(results[0][0].total_hours, 'HH:mm:ss').format(
                        'HH'
                      ) < 01
                      ? moment(results[0][0].total_hours, 'HH:mm:ss').format(
                          'mm'
                ...
                taskExactHours =
                  results[0][0].total_hours !== null
                    ? moment
                        .duration(results[0][0].total_hours, 'hours')
                        .format('HH:mm', {
                          forceLength: true
                        })
                    : 0
                meetingExactHours =
                  results[1][0].total_hours !== null
                    ? moment
                        .duration(results[1][0].total_hours, 'hours')
                        .format('HH:mm', {
                          forceLength: true
                        })
                    : 0
                ...
                budgetExactHours =
                  results[2][0].total_hours !== null
                    ? moment
                        .duration(results[2][0].total_hours, 'hours')
                        .format('HH:mm', {
                          forceLength: true
                        })
                    : 0
                ...
                meetingHours =
                  results[1][0].total_hours !== null
                    ? moment(results[1][0].total_hours, 'HH:mm:ss').format(
                        'HH'
                      ) < 01
                      ? moment(results[1][0].total_hours, 'HH:mm:ss').format(
                          'mm'
                ...
                budgetHours =
                  results[2][0].total_hours !== null
                    ? moment(results[2][0].total_hours, 'HH:mm:ss').format(
                        'HH'
                      ) < 01
                      ? moment(results[2][0].total_hours, 'HH:mm:ss').format(
                          'mm'
                ...
                obj.horasExatas =
                  parseInt(
                    moment
                      .duration(taskExactHours)
                      .add(meetingExactHours)
                      .add(budgetExactHours)
                      .format('HH:mm')
                  ) > 0
                    ? moment
                        .duration(taskExactHours)
...
router.get('/basic', checkToken, (req, res) => {
  jwt.verify(req.token, SECRET_KEY, (err, results) => {
    if (err) {
      ...
      connection.query(
        'Select id_client, name_client, monthly_hours_client from clients ORDER BY name_client ASC',
        function (error, results, fields) {
          if (error) throw error
          if (results.length > 0) {
            res.send(results)
...
router.get('/basic/:client', checkToken, (req, res) => {
  jwt.verify(req.token, SECRET_KEY, (err, results) => {
    if (err) {
      ...
      connection.query(
        'Select id_client, name_client, monthly_hours_client from clients WHERE id_client = ?',
        req.params.client,
        function (error, results, fields) {
          if (error) throw error
          if (results.length > 0) {
...
router.post('/', checkToken, (req, res) => {
  jwt.verify(req.token, SECRET_KEY, (err, results) => {
    if (err) {
      ...
      connection.query(
        'INSERT INTO clients (name_client, monthly_hours_client) VALUES (?,?)',
...
router.put('/', checkToken, (req, res) => {
  jwt.verify(req.token, SECRET_KEY, (err, results) => {
    if (err) {
      ...
      connection.query(
        'UPDATE clients SET name_client = ?, monthly_hours_client = ? WHERE id_client = ?',
