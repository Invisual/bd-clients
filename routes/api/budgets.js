...
router.post('/', checkToken, (req, res) => {
  jwt.verify(req.token, SECRET_KEY, (err, results) => {
    if (err) {
      ...
      connection.query(
        'INSERT INTO budgets (title_budget, description_budget, theme_budget) VALUES (?,?,?)',
        [
          req.body.title,
        ...
        function(error, results, fields) {
          ...
          connection.query(
            ...
            [req.body.user, results.insertId],
...
router.put('/', checkToken, (req, res) => {
  jwt.verify(req.token, SECRET_KEY, (err, results) => {
    if (err) {
      ...
      connection.query(
        'UPDATE budgets SET title_budget = ?, description_budget = ?,  creation_date_budget = ?, conclusion_date_budget = ?, ref_id_client = ?, theme_budget = ? WHERE id_budget = ?',
        [
          req.body.title,
          req.body.description,
          req.body.startdate,
          ...
          if (req.body.changeUser) {
            connection.query(
              ...
              function(error, results2, fields) {
                if (error) throw error;
                connection.query(
                  'INSERT INTO users_has_budgets (ref_id_user, ref_id_budgets) VALUES (?,?)',
