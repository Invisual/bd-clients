...
router.get('/', checkToken, (req, res) => {
  jwt.verify(req.token, SECRET_KEY, (err, results) => {
    if (err) {
      ...
      connection.query(
        'Select * from users INNER JOIN positions ON users.ref_id_position = positions.id_position WHERE status_user = 1 ORDER BY name_user',
...
router.post('/', checkToken, (req, res) => {
  jwt.verify(req.token, SECRET_KEY, (err, results) => {
    ...
    if (err) {
      ...
      connection.query(
        'INSERT INTO users (name_user, username_user, email_user, phone_user, avatar_user, password_user, status_user, ref_id_position, ref_id_role) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [
          req.body.name,
          req.body.username,
          req.body.email,
...
router.get('/accounts', checkToken, (req, res) => {
  jwt.verify(req.token, SECRET_KEY, (err, results) => {
    if (err) {
      ...
      //If token is successfully verified, we can send the autorized data
      connection.query(
        'Select id_user, name_user from users WHERE ref_id_position = 2 ORDER BY name_user',
        function (error, results, fields) {
          if (err) {
            throw err
          }
...
router.post('/login', (req, res) => {
  connection.query(
    'Select * from users INNER JOIN positions ON users.ref_id_position = positions.id_position WHERE username_user = ?',
    req.body.username,
    function (error, results, fields) {
      if (error) throw error
      if (results.length > 0) {
  ...
  connection.query(
    ...
    function (error, results, fields) {
      ...
      if (results.length > 0) {
        ...
        connection.query(
          'UPDATE users SET reset_password_user = ? WHERE id_user = ?',
          [randomString, idUser],
          function (error, results, fields) {
            if (error) throw error
...
router.get('/details/:user/:start/:end', checkToken, (req, res) => {
  ...
  jwt.verify(req.token, SECRET_KEY, (err, results) => {
    if (err) {
      ...
      connection.query(
        'SELECT * FROM users INNER JOIN positions ON users.ref_id_position = positions.id_position WHERE id_user=?',
        user,
        function (error, results, fields) {
          if (error) throw error
          if (results.length > 0) {
      ...
      connection.query(
        'SELECT * FROM user_infos  WHERE ref_id_user=?',
        user,
        function (error, results, fields) {
          if (error) throw error
          if (totalResults.details[0].id_user !== null) {
            totalResults.infos = results
          } else {
            res.send('nodata')
          }
        }
      ...
      connection.query(
        "SELECT * FROM vacations WHERE ref_id_user=? AND start_date LIKE '%?%'",
        [user, CURRYEAR],
        function (error, results, fields) {
          if (error) throw error
          if (totalResults.details[0].id_user !== null) {
            totalResults.vacations = results
          } else {
            res.send('nodata')
          }
        }
      ...
      connection.query(
        "SELECT id_budget, title_budget, budgets.ref_id_client, name_client, ref_id_budget_internal_status, SEC_TO_TIME(SUM(TIME_TO_SEC(TIMEDIFF(budget_hours.ending_hour, budget_hours.beginning_hour)))) AS 'total_budget_hours' FROM budgets LEFT JOIN users_has_budgets ON budgets.id_budget=users_has_budgets.ref_id_budget LEFT JOIN budget_hours ON budgets.id_budget = budget_hours.ref_id_budget LEFT JOIN users ON users_has_budgets.ref_id_user= users.id_user LEFT JOIN clients ON budgets.ref_id_client = clients.id_client WHERE users_has_budgets.ref_id_user=? AND (day BETWEEN ? AND ?) GROUP BY id_budget",
        [user, startDate, endDate],
        function (error, results, fields) {
          if (error) throw error
          if (totalResults.details[0].id_user !== null) {
      ...
      connection.query(
        'SELECT id_meeting_hour, id_meeting, beginning_hour, ending_hour, day, SEC_TO_TIME(SUM(TIME_TO_SEC(TIMEDIFF(ending_hour, beginning_hour)))) as difference from meeting_hours INNER JOIN meetings ON meeting_hours.ref_id_meeting = meetings.id_meeting WHERE ref_id_user = ? AND (day BETWEEN ? AND ?) GROUP BY id_meeting_hour ORDER BY id_meeting_hour ASC',
        [user, startDate, endDate],
        function (error, results, fields) {
          if (error) throw error
          if (totalResults.details[0].id_user !== null) {
      ...
      connection.query(
        'SELECT id_task_hour, id_task, beginning_hour, ending_hour, day, title_task, SEC_TO_TIME(SUM(TIME_TO_SEC(TIMEDIFF(ending_hour, beginning_hour)))) as difference from task_hours INNER JOIN tasks ON task_hours.ref_id_tasks = tasks.id_task WHERE ref_id_users = ? AND (day BETWEEN ? AND ?) AND ending_hour IS NOT NULL GROUP BY id_task_hour ORDER BY id_task_hour ASC',
        [user, startDate, endDate],
        function (error, results, fields) {
          if (error) throw error
...
router.get('/withtasks', checkToken, (req, res) => {
  ...
  jwt.verify(req.token, SECRET_KEY, (err, results) => {
    if (err) {
      ...
      connection.query(
        'SELECT id_user, name_user, avatar_user, ref_id_position, name_position, count(users_has_tasks.ref_id_task) AS total_tasks, count( CASE WHEN users_has_tasks.ref_id_user_task_status = 4 THEN ref_id_user_task_status ELSE NULL END) AS total_awaiting_tasks FROM users LEFT JOIN users_has_tasks ON users.id_user = users_has_tasks.ref_id_user LEFT JOIN positions ON users.ref_id_position = positions.id_position WHERE ref_id_position != 1 AND ref_id_position != 3 GROUP BY id_user',
        function (error, results, fields) {
          if (error) throw error
          if (results.length > 0) {
            totalResults.users = results
          }
        }
      )
      connection.query(
        ...
        function (error, results, fields) {
          if (error) throw error
          totalResults.tasks = results
          if (totalResults.users[0].id_user !== null) {
            res.send(totalResults)
          } else {
            res.send('nodata')
          }
...
router.post('/info', checkToken, (req, res) => {
  jwt.verify(req.token, SECRET_KEY, (err, results) => {
    if (err) {
      ...
      connection.query(
        'INSERT INTO user_infos (user_info_title, user_info_email, user_info_password, user_info_obs, ref_id_user) VALUES(?, ?, ?, ?, ?)',
        [
          req.body.infoTitle,
          req.body.infoEmail,
          req.body.infoPassword,
          req.body.infoObs,
          req.body.activeMemberId
        ],
        function (error, results, fields) {
          if (error) throw error
          res.send(results)
...
router.get('/verifyrandomstring', (req, res) => {
  connection.query(
    'Select reset_password_user from users WHERE id_user = ?',
    req.body.iduser,
    function (error, results, fields) {
      if (error) throw error
      if (results.length > 0) {
        results[0].reset_password_user === req.body.randomstring
          ? res.send('correctstring')
          : res.send('badstring')
      } else {
        res.send('baduser')
      }
...
router.put('/changepassword', (req, res) => {
  var encPassword = bcrypt.hashSync(req.body.password)
  connection.query(
    "UPDATE users SET password_user = ?, reset_password_user = '' WHERE id_user = ?",
    [encPassword, req.body.iduser],
    function (error, results, fields) {
      if (error) throw error
      res.send(results)
    }
...
router.put('/inactive/:user', (req, res) => {
  jwt.verify(req.token, SECRET_KEY, (err, results) => {
    connection.query(
      "UPDATE users SET status_user = '0' WHERE id_user = ?",
      req.params.user,
      function (error, results, fields) {
        if (error) throw error
        res.send(results)
...
router.get('/:user', checkToken, (req, res) => {
  jwt.verify(req.token, SECRET_KEY, (err, results) => {
    if (err) {
      ...
      //If token is successfully verified, we can send the autorized data
      connection.query(
        'Select * from users INNER JOIN positions ON users.ref_id_position = positions.id_position WHERE status_user = 1 AND id_user = ?',
        req.params.user,
        function (error, results, fields) {
          if (err) {
            throw err
  ...
  jwt.verify(req.token, SECRET_KEY, (err, results) => {
    if (err) {
      ...
      if (req.body.changePassword) {
        var encPassword = bcrypt.hashSync(req.body.password)
        connection.query(
          'UPDATE users SET name_user = ?, username_user = ?, email_user = ?, phone_user = ?, avatar_user = ?, password_user = ?, ref_id_position = ? WHERE id_user = ?',
          [
            req.body.name,
            req.body.username,
            req.body.email,
        ...
        connection.query(
          'UPDATE users SET name_user = ?, username_user = ?, email_user = ?, phone_user = ?, avatar_user = ?, ref_id_position = ? WHERE id_user = ?',
          [
            req.body.name,
            req.body.username,
            req.body.email,
